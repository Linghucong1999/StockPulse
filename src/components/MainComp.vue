...
import NewsPanelComp from './NewsPanelComp.vue'
import stocksData from '../data/stocks.json'
import { searchLocal, stockName } from '../utils/search'
import { sma, calcMACD, C_UP, C_DOWN, C_MA5, C_MA20, C_MA60 } from '../utils/stats'
...
const emit = defineEmits(['update:query', 'search', 'select', 'period-change'])

let chart = null
let histChart = null
let corrChart = null
const histBins = ref(20)

const dayChange = computed(() => {
  const p = props.meta.regularMarketPrice
  const prev = props.meta.chartPreviousClose
...
const chgCls = computed(() => {
  const d = dayChange.value
  return d > 0 ? 'up' : d < 0 ? 'down' : 'flat'
})
const trendTagType = computed(() =>
  ({ '上升趋势': 'danger', '下降趋势': 'success', '震荡整理': 'info' }[props.stats.trend] || 'info'))
const rangeLabel = computed(() => PERIODS[props.period].range)
...
function maCls(n) { return (props.stats['ma' + n] || 0) >= (props.meta.regularMarketPrice || 0) ? 'down' : 'up' }
function signCls(v) { return v > 0 ? 'up' : v < 0 ? 'down' : 'flat' }
function rsiCls() { const r = props.stats.rsi; return r > 70 ? 'up' : r < 30 ? 'down' : 'flat' }
function exchName(exch) {
  return ({
...
function fieldName(i) { return ['开', '高', '低', '收', '量'][i] || '' }
function fmtCorr(v) { return (v == null || isNaN(v)) ? '—' : v.toFixed(2) }
...
function querySuggestions(queryString, cb) {
  cb(searchLocal(queryString).slice(0, 8).map(x => ({ value: x.cn || x.en || x.code, code: x.code, cn: x.cn, en: x.en })))
}
function onSelect(item) {
  emit('update:query', item.value)
...
function renderChart() {
  ...
  chart.setOption({
    ...
    series: [
      { name: 'K线', type: 'candlestick', data: kData, itemStyle: { color: C_UP, color0: C_DOWN, borderColor: C_UP, borderColor0: C_DOWN } },
      ...
      { name: '成交量', type: 'bar', xAxisIndex: 1, yAxisIndex: 1, data: vols, itemStyle: { color: (p) => candles[p.dataIndex].c >= candles[p.dataIndex].o ? 'rgba(255,77,106,0.55)' : 'rgba(46,204,113,0.55)' } },
      { name: 'MACD', type: 'bar', xAxisIndex: 2, yAxisIndex: 2, data: macd.hist.map((v) => ({ value: v, itemStyle: { color: v >= 0 ? 'rgba(255,77,106,0.7)' : 'rgba(46,204,113,0.7)' } })), barWidth: '60%' },
    ],
  }, { notMerge: true })
}
...
function renderHistogram() {
  const closes = props.candles.map(x => x.c)
  if (!closes.length) return
  const el = document.getElementById('hist')
  if (!histChart) histChart = echarts.init(el)
  const bins = histBins.value
  ...
  const statDefs = [
    { v: props.stats.mean, name: '均值', color: '#f5c542', pos: 'end' },
    { v: props.stats.median, name: '中位数', color: '#4fc3f7', pos: 'start' },
  ]
  const markData = statDefs.filter(d => d.v != null && !isNaN(d.v)).map(d => ({
    xAxis: binOf(d.v), name: d.name + ' ' + fmtShort(d.v),
    lineStyle: { color: d.color, width: 1, type: 'dashed' },
    label: { color: d.color, fontSize: 10, formatter: '{b}', position: d.pos, rotate: 0, overflow: 'none' },
  }))
  const modeIdx = (props.stats.mode != null && !isNaN(props.stats.mode)) ? binOf(props.stats.mode) : -1
  ...
  histChart.setOption({
    ...
    tooltip: {
      ...
      textStyle: { color: '#f4f4f8' },
      formatter: (p) => {
        const x = labels[p.dataIndex] != null ? labels[p.dataIndex] : p.axisValue
        return '<b>横轴(价格区间): ' + x + '</b><br/>纵轴(频数): ' + p.value
      },
    },
    grid: { left: 44, right: 16, top: 24, bottom: 26 },
    xAxis: { type: 'category', data: labels, axisLabel: { color: '#9a9ab0', fontSize: 10, interval: Math.max(0, Math.ceil(bins / 10) - 1) }, axisLine: { lineStyle: { color: 'rgba(255,255,255,0.15)' } } },
    yAxis: { type: 'value', name: '频数', nameTextStyle: { color: '#9a9ab0', fontSize: 10 }, splitLine: { lineStyle: { color: 'rgba(255,255,255,0.06)' } }, axisLabel: { color: '#9a9ab0', fontSize: 10 } },
    series: [{
      ...
      label: { show: true, position: 'top', color: '#ab8cff', fontSize: 10, fontWeight: 600, formatter: (p) => (p.dataIndex === modeIdx ? '众数 ' + fmtShort(props.stats.mode) : '') },
...
function renderCorrHeatmap() {
  const corr = props.stats.matrix.corr
  if (!corr || !corr.length) return
  const el = document.getElementById('corr-heat')
  if (!corrChart) corrChart = echarts.init(el)
  const fields = ['开', '高', '低', '收', '量']
  const data = []
  for (let i = 0; i < corr.length; i++) {
    for (let j = 0; j < corr[i].length; j++) data.push([j, i, corr[i][j]])
  }
  ...
  corrChart.setOption({
    ...
    xAxis: { type: 'category', data: fields, axisLabel: { color: '#9a9ab0', fontSize: 10 }, splitArea: { show: true, areaStyle: { color: ['rgba(255,255,255,0.02)', 'rgba(255,255,255,0.05)'] } } },
    yAxis: { type: 'category', data: fields, axisLabel: { color: '#9a9ab0', fontSize: 10 }, splitArea: { show: true, areaStyle: { color: ['rgba(255,255,255,0.02)', 'rgba(255,255,255,0.05)'] } } },
    visualMap: {
      min: -1, max: 1, calculable: false, orient: 'horizontal', left: 'center', bottom: 0,
      itemWidth: 8, itemHeight: 80, textStyle: { color: '#9a9ab0', fontSize: 10 },
      inRange: { color: [downColor, '#12121c', upColor] },
    },
    series: [{
      type: 'heatmap', data,
...
watch(() => props.stats, () => { renderHistogram(); renderCorrHeatmap() })
...
onMounted(() => {
  ...
  _onResize = () => { if (chart) chart.resize(); if (histChart) histChart.resize(); if (corrChart) corrChart.resize() }
  ...
  <div class="main-scroll">
    <section class="hero">
      ...
      <div class="hero-search">
        <el-autocomplete :model-value="query" :fetch-suggestions="querySuggestions"
                         class="header-search" placeholder="输入中文名/代码/拼音，例如 茅台 / 600519 / maotai / AAPL"
                         clearable popper-class="stock-suggest"
                         @select="onSelect" @update:model-value="$emit('update:query', $event)"
          ...
          <div class="stats-row">
            ...
            <div class="stat-col">
              ...
              <table class="ind-table">
                ...
                <tr><td><el-tooltip content="MACD 柱 = 2 × (DIF − DEA)，柱 &gt; 0 表示多头动能增强，&lt; 0 为空头动能增强。" placement="top"><span class="ind-name">MACD 柱</span></el-tooltip></td><td :class="signCls(stats.macd.hist)">{{ fmt(stats.macd.hist) }}</td></tr>
                ...
                <tr><td><el-tooltip content="综合短期均线（MA5/MA20）与 MACD 动能得出的趋势方向。" placement="top"><span class="ind-name">趋势判定</span></el-tooltip></td><td><el-tag :type="trendTagType" size="small" effect="dark">{{ stats.trend }}</el-tag></td></tr>
              </table>
            </div>
          </div>
          ...
          <div class="advice-box">
            ...
            <div class="advice-row">
              <div class="advice-item">
                ...
                <el-tag :type="stats.longAdvice ? 'success' : 'info'" size="small" effect="dark">{{ stats.longAdvice ? '适合长期持有' : '暂不建议长期' }}</el-tag>
              </div>
              <div class="advice-item">
                <span class="advice-label">短期评分</span>
                <span class="advice-score">{{ stats.shortScore }}/4</span>
                <el-tag :type="stats.shortAdvice ? 'warning' : 'info'" size="small" effect="dark">{{ stats.shortAdvice ? '适合短期持有' : '暂不建议短期' }}</el-tag>
              </div>
            </div>
